...
```text
Please install the `paper-to-skill` package from https://github.com/VeryMath/AI4Math-Paper-Reading.git (branch: main, subdirectory: skills/paper-to-skill). Read `.agent.md`, install the declared Skill entrypoint, verify that `$paper-to-skill` is discoverable, and tell me whether I need to restart the agent.
```
